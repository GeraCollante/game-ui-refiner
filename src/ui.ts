...
export function setLoading(on: boolean, text = 'querying...'): void {
...
export function updateMeters(): void {
...
export function startTicker(): void {
  ...
  const tick = () => {
    if (state.runStartTime == null) return;
...
export function log(level: LogEntry['level'], msg: string, details: Partial<LogEntry> = {}): void {
  const entry: LogEntry = { ts: new Date().toISOString().slice(11, 23), level, msg, ...details };
  ...
  if (details.model) line += ` <span class="text-neutral-500">(${details.model})</span>`;
  if (details.dt != null) line += ` <span class="text-neutral-500">${details.dt.toFixed(2)}s</span>`;
  if (details.cost != null) line += ` <span class="text-amber-400">$${details.cost.toFixed(5)}</span>`;
  if (details.tokens) line += ` <span class="text-neutral-500">${details.tokens.in}→${details.tokens.out}tk</span>`;
  div.innerHTML = line;

  const pane = $('paneLogs');
  pane.appendChild(div);
  while (pane.childNodes.length > MAX_LOG_ENTRIES) pane.removeChild(pane.firstChild!);
...
export function updateCodePanes(svelte: string | null, html: string | null): void {
...
function buildMessagesDom(messages: ChatMessage[] | null, container: HTMLElement): void {
  if (!messages) {
...
function buildSection(title: string, color: string, messages: ChatMessage[] | null, rawResponse: string): HTMLElement {
...
export function refreshPromptsPane(force = false): void {
  ...
  if (state.lastPrompts.generator || state.lastResponses.generator) {
    pane.appendChild(
      buildSection('████████ GENERATOR ████████', 'text-purple-400', state.lastPrompts.generator, state.lastResponses.generator)
    );
  }
  if (state.lastPrompts.critic || state.lastResponses.critic) {
    pane.appendChild(
      buildSection('████████ CRITIC ████████', 'text-cyan-400', state.lastPrompts.critic, state.lastResponses.critic)
    );
  }
}
...
export function drawChart(): void {
  ...
  if (n === 0) {
    svg.insertAdjacentHTML(
      'beforeend',
      `<text x="160" y="90" fill="#525252" font-size="11" text-anchor="middle">esperando datos...</text>`
    );
    return;
  }
  ...
  const yFor = (v: number) => PAD_T + plotH - (v / 10) * plotH;
  const xFor = (i: number) => PAD_L + (n > 1 ? i * stepX : plotW / 2);
  ...
  for (let g = 0; g <= 10; g += 2) {
    ...
    svg.insertAdjacentHTML(
      'beforeend',
      `<line x1="${PAD_L}" y1="${y}" x2="${PAD_L + plotW}" y2="${y}" stroke="#262626" stroke-width="0.5" />`
    );
    svg.insertAdjacentHTML(
      'beforeend',
      `<text x="${PAD_L - 4}" y="${y + 3}" fill="#737373" font-size="9" text-anchor="end" font-family="monospace">${g}</text>`
    );
  }
  ...
  svg.insertAdjacentHTML(
    'beforeend',
    `<line x1="${PAD_L}" y1="${PAD_T + plotH}" x2="${PAD_L + plotW}" y2="${PAD_T + plotH}" stroke="#404040" stroke-width="0.8" />`
  );
  for (let i = 0; i < n; i++) {
    svg.insertAdjacentHTML(
      'beforeend',
      `<text x="${xFor(i)}" y="${H - 8}" fill="#737373" font-size="9" text-anchor="middle" font-family="monospace">e${i + 1}</text>`
    );
  }
  ...
  for (const [dim, info] of Object.entries(dimsToPlot)) {
    ...
    state.scoreHistory.forEach((s, i) => {
      const v = clamp01_10((s.scores as any)?.[dim]);
      if (v == null) {
    ...
    for (const seg of segs) {
      if (seg.length >= 2) {
        svg.insertAdjacentHTML(
          'beforeend',
          `<polyline points="${seg.join(' ')}" fill="none" stroke="${color}" stroke-width="1.8" />`
        );
      }
    }
    ...
    state.scoreHistory.forEach((s, i) => {
      ...
      svg.insertAdjacentHTML(
        'beforeend',
        `<circle cx="${xFor(i)}" cy="${yFor(v)}" r="2.5" fill="${color}"><title>${dim} epoch ${i + 1}: ${v.toFixed(1)}</title></circle>`
      );
    });

    const legendY = PAD_T + 4 + legendIdx * 12;
    const valTxt = lastValid == null ? '—' : (lastValid as number).toFixed(1);
    svg.insertAdjacentHTML(
      'beforeend',
      `<rect x="${PAD_L + plotW + 6}" y="${legendY}" width="6" height="6" fill="${color}" />`
    );
    svg.insertAdjacentHTML(
      'beforeend',
      `<text x="${PAD_L + plotW + 16}" y="${legendY + 6}" fill="#a3a3a3" font-size="9" font-family="monospace">${info.short} <tspan fill="${color}" font-weight="bold">${valTxt}</tspan></text>`
    );
    legendIdx++;
  }
  ...
  if (overallSeg.length >= 2) {
    svg.insertAdjacentHTML(
      'beforeend',
      `<polyline points="${overallSeg.join(' ')}" fill="none" stroke="#fafafa" stroke-width="1.5" stroke-dasharray="3,2" opacity="0.7" />`
    );
  }
  if (overallSeg.length >= 1) {
    ...
    if (lastOverall != null) {
      ...
      svg.insertAdjacentHTML(
        'beforeend',
        `<line x1="${PAD_L + plotW + 6}" y1="${legendY + 3}" x2="${PAD_L + plotW + 12}" y2="${legendY + 3}" stroke="#fafafa" stroke-width="1.5" stroke-dasharray="2,1" />`
      );
      svg.insertAdjacentHTML(
        'beforeend',
        `<text x="${PAD_L + plotW + 16}" y="${legendY + 6}" fill="#a3a3a3" font-size="9" font-family="monospace">overall <tspan fill="#fafafa" font-weight="bold">${lastOverall.toFixed(1)}</tspan></text>`
      );
    }
  }
}
...
export function addHistoryThumb(
  epoch: number | string,
  dataUrl: string | null,
  html: string,
  svelte: string,
  ...
  while (historyEl.childNodes.length > MAX_HISTORY_THUMBS) historyEl.removeChild(historyEl.firstChild!);
...
export function populateDropdown(selectId: string, options: string[]): void {
...
export function refreshProviderUI(): void {
  ...
  (($('preset') as HTMLSelectElement)).value = localStorage.getItem(presetKey) || 'smart';
  applyPreset(($('preset') as HTMLSelectElement).value);
}
...
export function applyPreset(name: string): void {
  ...
  if (!p) return;
  ($('criticModel') as HTMLSelectElement).value = p.critic;
  ($('genModel') as HTMLSelectElement).value = p.gen;
...
export async function saveToServer(filename: string, body: Record<string, string>): Promise<string | null> {
  if (!state.currentSession) return null;
  if (!($('saveToggle') as HTMLInputElement).checked) return null;
  try {
...
export async function saveImage(filename: string, dataUrl: string | null): Promise<string | null> {
 ...
 */
export function buildExportMarkdown(): string {
  const lines: string[] = [];
  const provider = currentProvider();
  const critic = (document.getElementById('criticModel') as HTMLSelectElement)?.value || '?';
  const gen = (document.getElementById('genModel') as HTMLSelectElement)?.value || '?';
  ...
  if (lastOverall != null) lines.push(`- **Final overall score**: ${lastOverall}/10`);
  lines.push('');
  ...
  const sysGen = mode === 'decompose'
    ? (document.getElementById('sysPromptDecomposeGen') as HTMLTextAreaElement | null)?.value
    : (document.getElementById('sysPromptGen') as HTMLTextAreaElement | null)?.value;
  const sysCritic = mode === 'decompose'
    ? (document.getElementById('sysPromptDecomposeCritic') as HTMLTextAreaElement | null)?.value
    : (document.getElementById('sysPromptCritic') as HTMLTextAreaElement | null)?.value;
  if (sysGen) {
